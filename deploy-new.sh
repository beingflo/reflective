...
( cd service ; version=$(cargo metadata --format-version=1 --no-deps | jq '.packages[0].version' | tr -d '"') )
...
( cd service ; cargo set-version "${new_version}" || die "Failed to set version in Cargo.toml" )
